...
const JobDetailsCard = ({ isFreelancer }) => {
	return (
		<div className="sm:w-2/3 w-full">
			<div className="flex flex-col sm:flex-row  justify-center bg-[#492772] bg-opacity-70 sm:w-auto sm:space-x-36 rounded-lg border border-purple-600 sm:p-10">
				...
				<div className="flex flex-wrap flex-col  space-y-3 sm:gap-y-64 sm:pt-12 py-3">
					<div className="space-y-6 sm:w-40 w-full">
						{isFreelancer ? (
							...
							<button class="flex items-center bg-white border border-gray-300 rounded-lg p-2 ml-4">
								<div class="h-6 w-6 bg-green-500 rounded-full"></div>
								<span class="sm:ml-2 text-[#4301a3] px-2 opacity-1 font-semibold">
									ACTIVE
								</span>
							</button>
						)}
