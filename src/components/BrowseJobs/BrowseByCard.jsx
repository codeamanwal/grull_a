...
const BrowseByCard = ({ topic, items, setFilter }) => {
  ...
  return (
    <div className="sm:my-6 ">
      <p className="text-white font-semibold sm:text-sm lg:text-xl sm:pb-6">BROWSE BY {topic}</p>
      <div className="flex flex-col sm:b-10">
        {isMobile ? (
          <div className="flex items-center my-2 gap-x-5">
            <button className="flex items-center gap-x-2 text-black font-medium sm:text-2xl" onClick={handleMenuClick}>
            ...
            {showOptions && (
              <div className="flex flex-col">
                {Object.keys(items).map((originalItem, index) => (
                  <div key={index} className="flex items-center my-2 gap-x-5">
                    <input
                      type="checkbox"
                      checked={checkedItems.includes(originalItem)}
                    ...
                    />
                    <span className="text-black font-medium sm:text-2xl">{originalItem}</span>
                  </div>
                ))}
              </div>
            )}
